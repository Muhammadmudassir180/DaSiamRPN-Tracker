...
def tracker_eval(net, x_crop, target_pos, target_sz, window, scale_z, p):
    ...
    delta = delta.permute(1, 2, 3, 0).contiguous().view(
        4, -1).data.cpu().numpy()
    score = F.softmax(score.permute(1, 2, 3, 0).contiguous().view(
...
def SiamRPN_init(im, target_pos, target_sz, net):
    ...
    p.anchor = generate_anchor(
        p.total_stride, p.scales, p.ratios, int(p.score_size))
...
def SiamRPN_track(state, im):
    ...
    x_crop = Variable(get_subwindow_tracking(
        im, target_pos, p.instance_size, round(s_x), avg_chans).unsqueeze(0))
